...
SWEP.PrintName = "Iron Maiden Assault rifle"
...
SWEP.Damage = 27 -- Damage per bullet, gets divided by SWEP.Count internally (input final damage, not per-bullet damage)
...
SWEP.MoveSpread = 1.5 -- Same unit as SWEP.BaseSpread, added when moving

SWEP.MoveSpeed = 0.4 -- Movespeed multiplier: 1 = run speed, 0 = alt-walk speed

SWEP.Recoil = {
	Kick = Angle(0.47, 0.5), -- View kick

	-- Viewmodel kick
	HipOffset = Vector(-5, 0, -1),
	HipPitch = 1.5,

	AimOffset = Vector(-3, 0, -0.4),
	AimPitch = 2,
...
SWEP.Sights = {
	...
	Time = 0.4, -- Time it takes to zoom in, also affects sprint and deploy times even if sights are disabled
	Zoom = 1.5,
	Distance = 17 -- Distance from the attachment point on the weapon model
